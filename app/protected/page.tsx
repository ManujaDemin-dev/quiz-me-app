...
export default function ProtectedDashboard() {
  ...
  // Fetch current user ID and quizzes on mount
  useEffect(() => {
    async function init() {
      try {
        // Get current user User data is from auth.users i just created a instant for the profile
        ...
        // Fetch quizzes from the quizzes table
  ...
  // Split quizzes into own and others'
  const myQuizzes = quizzes.filter((q) => q.creator_id === userId);
  const otherQuizzes = quizzes.filter((q) => q.creator_id !== userId);
  ...
  const handleCreateQuiz = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // route
        router.push(`/protected/quiz/${createdQuiz.id}`);
      } else {
    ...
    <div className="min-h-screen bg-[#0B0B0C] text-white p-6 md:p-12 space-y-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-white/5">
        <div>
          ...
          <p className="text-xs text-[#61616A] mt-1">
            Deploy mathematical evaluation instances or test your optimal
            limits.
          </p>
          {/* these value are placeholder values i just want to calrify  */}
        </div>
        <div className="flex gap-3">
          <Link
            href="/profile"
            className="px-4 py-2.5 bg-[#1A1A1E] text-xs font-semibold rounded-xl border border-white/5 hover:bg-[#222227] transition"
          >
            My Profile Settings
          </Link>
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#141416] border border-white/5 rounded-[20px] p-6 space-y-4">
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-[#61616A]">
              Enter Room Code
            </h3>
            <p className="text-xs text-[#A1A1AA] mt-1">
              Have a specific challenge key? Drop the UUID string here.
            </p>
          </div>
          <form onSubmit={handleJoinRoom} className="flex gap-2">
            ...
            <button
              type="submit"
              className="bg-[#222227] hover:bg-[#2c2c32] px-4 py-2 rounded-xl text-xs font-semibold border border-white/5 transition"
            >
              Join
            </button>
          </form>
        ...
        {showCreateForm && (
          <form
            ...
            className="lg:col-span-2 bg-[#141416] border border-white/5 rounded-[20px] p-6 space-y-4 animate-fadeIn"
          >
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={creating}
                className="bg-white text-background font-bold text-xs px-5 py-2.5 rounded-xl disabled:opacity-50"
              >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {myQuizzes.map((quiz, idx) => (
              <div
                key={quiz.id}
                className="bg-[#141416] border border-white/5 hover:border-white/10 rounded-[20px] p-6 flex flex-col justify-between space-y-4 transition-all group animate-fadeSlideIn"
                style={{ animationDelay: `${idx * 0.05}s` }}
              >
                <div>
                  <div className="flex justify-between items-start gap-2">
                    <h3 className="font-semibold text-white group-hover:text-white/90 tracking-tight text-md line-clamp-1">
                      {quiz.title}
                    </h3>
                    <span
                      className={`text-[10px] font-mono px-2 py-0.5 rounded border uppercase tracking-wide ${difficultyStyle(quiz.difficulty)}`}
                    >
                      {quiz.difficulty}
                    </span>
                  </div>
                  <p className="text-xs text-[#A1A1AA] mt-2 line-clamp-2 min-h-[32px]">
                    {quiz.description || "No layout description provided."}
                  </p>
                </div>
                ...
                <div className="pt-4 border-t border-white/5 space-y-3">
                  <div className="bg-[#1A1A1E] px-3 py-2 rounded-xl border border-white/5 text-[10px] font-mono flex flex-col gap-0.5">
                    ...
                    <span className="text-[#A1A1AA] select-all truncate">
                      {quiz.room_code}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <Link
                      ...
                      className="text-center bg-white text-[#0B0B0C] font-bold text-xs py-2.5 rounded-xl hover:bg-white/90 transition"
                    >
                      Manage →
                    </Link>
                    <Link
                      href={`/quiz/room/${quiz.room_code}`}
                      className="text-center bg-[#222227] hover:bg-[#2c2c32] font-semibold text-xs py-2.5 rounded-xl border border-white/5 transition"
                    >
                      Preview
                    </Link>
                  </div>
      ...
      <div className="space-y-4">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-[#61616A]">
          {myQuizzes.length > 0
            ? "Other Public Arenas"
            : "Available Public Arenas"}
        </h2>

        {loading ? (
          <div className="text-xs text-[#A1A1AA] animate-pulse">
            Synchronizing feed items...
          </div>
          ...
          <div className="text-xs text-[#61616A] italic bg-[#141416] p-6 rounded-[20px] text-center border border-white/5">
            No other public quizzes available right now.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
